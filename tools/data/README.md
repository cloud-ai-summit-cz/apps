...
### 3. Authenticate (for import scripts)
...
# Clean all toys (with Admin.FullAccess role support)
...
# Clean all trips (with Admin.FullAccess role support)
...
# Authentication
AUTH_TOKEN_PATH=../identity/auth_token.json
...
- **Import scripts need services**: Only the import scripts require running services and auth tokens
...
Both `clean-toy-profiles.py` and `clean-trip-profiles.py` support the `Admin.FullAccess` role:
- **Regular users**: Can only delete their own toys/trips
- **Admin users**: Can delete ALL toys/trips regardless of owner
