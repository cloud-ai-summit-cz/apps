...
class TripApiClient {
  ...
  async createTrip(data: TripCreate): Promise<Trip> {
    const response = await this.fetchWithAuth(`${this.baseUrl}/trip`, {
      method: 'POST',
      headers: {
  ...
  async getTrip(id: string): Promise<Trip> {
    const response = await this.fetchWithAuth(`${this.baseUrl}/trip/${id}`);
  ...
  async listTrips(params: {
    ...
    const response = await this.fetchWithAuth(`${this.baseUrl}/trip?${queryParams}`);
  ...
  async getGalleryImageBlob(tripId: string, imageId: string): Promise<string> {
    const response = await this.fetchWithAuth(`${this.baseUrl}/trip/${tripId}/gallery/${imageId}`);
