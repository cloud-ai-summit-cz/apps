...
* Trip creation to single destinations (location + optional landmark visits)
* Gallery images from destinations (uploaded or generated demo images)
...
## 4. Glossary
* Trip: A journey to a single destination for one toy.
* Destination: The location where the trip takes place (e.g., Paris, Tokyo).
* Landmark: A specific point of interest within the destination that can be featured in gallery images.
* Add‑On: Accessory (visual item) OR experience (ephemeral activity) enhancing a trip.
* Gallery Image: Media asset from the destination (uploaded or generated).
...
10. Chat Current Status: Owner asks "Where is my toy now?" and receives current location + last image + trip status.
